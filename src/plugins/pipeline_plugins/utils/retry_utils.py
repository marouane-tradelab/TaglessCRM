...
def _is_retriable_http_error(error: errors.HttpError) -> bool:
  ...
  if ('resp' in error.__dict__ and
      error.__dict__['resp'].status in RETRY_UTILS_RETRIABLE_STATUS_CODES):
    return True
  return False
...
def _is_retriable_http_airflow_exception(
  ...
  defined in RETRY_UTILS_RETRIABLE_STATUS_CODES.
  ...
  try:
    if int(status_code) in RETRY_UTILS_RETRIABLE_STATUS_CODES:
      return True
    return False
  except ValueError:
    return False
