...
"""Tests for dags.bq_to_ads_cm_dag_v2."""

import unittest

from dags import bq_to_ads_cm_dag_v2
import dag_test
...
class DAGTest(dag_test.DAGTest):
  ...
  def setUp(self):
    ...
    self.init_airflow_variables(bq_to_ads_cm_dag_v2._DAG_NAME)

  def test_create_dag(self):
    """Tests that returned DAG contains correct DAG and tasks."""
    expected_task_ids = ['bq_to_ads_cm_retry_task', 'bq_to_ads_cm_task']

    is_var_prefixed = False
    test_dag = bq_to_ads_cm_dag_v2.BigQueryToAdsCMV2Dag(
    ...
    self.verify_ads_cm_hook_v2(test_dag.tasks[1].output_hook, is_var_prefixed)
