...
"""Tests for dags.gcs_to_ga4_dag."""

import unittest

import dag_test
from dags import gcs_to_ga4_dag
...
class DAGTest(dag_test.DAGTest):
  ...
  def setUp(self):
    ...
    self.init_airflow_variables(gcs_to_ga4_dag._DAG_NAME)

  def test_create_dag(self):
    """Tests that returned DAG contains correct DAG and tasks."""
    expected_task_ids = ['gcs_to_ga4_retry_task', 'gcs_to_ga4_task']

    is_var_prefixed = False
    test_dag = gcs_to_ga4_dag.GCSToGA4Dag(
    ...
    self.verify_ga4_hook(test_dag.tasks[1].output_hook, is_var_prefixed)
